...
('Harry Potter e a Pedra Filosofal', 1997, 180, 4, 1, 102, 1, 'Editora D'),
('Cem Anos de Solidão', 1967, 120, 5, 1, 103, 1, 'Editora E');
...
1. **SELECT s.tipo_servico, COUNT(l.id_livro) AS total_por_tipo**
   - `SELECT`: Esta é a parte da consulta onde selecionamos as colunas que queremos retornar.
   - `s.tipo_servico`: Selecionamos o tipo de serviço da tabela `servico` e renomeamos como `tipo_servico`.
   - `COUNT(l.id_livro) AS total_por_tipo`: Usamos a função de agregação `COUNT()` para contar o número de livros para cada tipo de serviço. Renomeamos essa contagem como `total_por_tipo`.

2. **FROM livro l**
   - `FROM`: Aqui especificamos de onde estamos obtendo os dados.
   - `livro l`: Estamos selecionando dados da tabela `livro` e a nomeando como `l`.

3. **JOIN servico s ON l.fk_servico_id_servico = s.id_servico**
   - `JOIN`: Estamos realizando uma junção entre as tabelas `livro` e `servico`.
   - `ON`: Aqui especificamos a condição de junção.
   - `l.fk_servico_id_servico = s.id_servico`: Estamos especificando que queremos juntar as linhas onde o ID do serviço em `livro` corresponde ao ID do serviço em `servico`.

4. **GROUP BY s.tipo_servico**
   - `GROUP BY`: Aqui estamos agrupando os resultados pelo tipo de serviço, para que possamos contar o número de livros para cada tipo de serviço.

5. **ORDER BY total_por_tipo DESC;**
   - `ORDER BY`: Estamos ordenando os resultados com base no número total de livros por tipo de serviço.
   - `total_por_tipo DESC`: Estamos ordenando em ordem decrescente com base no total de livros por tipo de serviço.
