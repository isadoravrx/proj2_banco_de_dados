...
1. **SELECT**
   - `SELECT`: Esta é a parte da consulta onde especificamos o que queremos retornar.

2. **(SELECT COUNT(*) FROM presencial) AS alunos_presenciais**
   - `(SELECT COUNT(*) FROM presencial)`: Aqui estamos realizando uma subconsulta para contar o número de registros na tabela `presencial`. Isso nos dará a quantidade de alunos cursando presencialmente.
   - `AS alunos_presenciais`: Renomeamos o resultado da subconsulta como `alunos_presenciais`.

3. **(SELECT COUNT(*) FROM EAD) AS alunos_EAD**
   - `(SELECT COUNT(*) FROM EAD)`: Novamente, estamos realizando uma subconsulta para contar o número de registros na tabela `EAD`. Isso nos dará a quantidade de alunos cursando a modalidade EAD.
   - `AS alunos_EAD`: Renomeamos o resultado da subconsulta como `alunos_EAD`.
