...
(null, 'Dinheiro', 100, 3, 100, 21213, 111, 1004, 1007),
(null, 'Dinheiro', 59, 4, 59, 21213, 111, 1004, 1007);
...
FROM 
    venda v
JOIN 
    pessoa p ON v.fk_cliente_fk_pessoa_id = p.id
GROUP BY 
    v.fk_cliente_fk_pessoa_id, p.nome;


explicação:
1. **SELECT metodo_pagamento, AVG(valor_total) AS valor_medio_compra**
   - `SELECT metodo_pagamento`: Aqui estamos selecionando a coluna `metodo_pagamento` da tabela `venda`, que representa o método de pagamento utilizado.
   - `AVG(valor_total) AS valor_medio_compra`: Estamos calculando a média dos valores totais de compra (`valor_total`) para cada método de pagamento e renomeando-a como `valor_medio_compra`.

2. **FROM venda**
   - `FROM venda`: Especificamos que os dados serão selecionados da tabela `venda`.

3. **GROUP BY metodo_pagamento**
   - `GROUP BY metodo_pagamento`: Estamos agrupando os resultados com base no método de pagamento, ou seja, estamos agrupando as vendas por tipo de pagamento.

4. **SELECT v.fk_cliente_fk_pessoa_id AS id_cliente, p.nome AS nome_cliente, AVG(v.valor_total) AS valor_medio_compra**
   - `SELECT v.fk_cliente_fk_pessoa_id AS id_cliente`: Aqui estamos selecionando o ID do cliente (`fk_cliente_fk_pessoa_id`) da tabela `venda` e renomeando-o como `id_cliente`.
   - `p.nome AS nome_cliente`: Estamos selecionando o nome do cliente da tabela `pessoa` e renomeando-a como `nome_cliente`.
   - `AVG(v.valor_total) AS valor_medio_compra`: Estamos calculando a média dos valores totais de compra (`valor_total`) para cada cliente e renomeando-a como `valor_medio_compra`.

5. **FROM venda v**
   - `FROM venda v`: Estamos especificando que os dados serão selecionados da tabela `venda` e a estamos renomeando como `v`.

6. **JOIN pessoa p ON v.fk_cliente_fk_pessoa_id = p.id**
   - `JOIN pessoa p ON v.fk_cliente_fk_pessoa_id = p.id`: Estamos juntando a tabela `venda` com a tabela `pessoa` com base no ID do cliente, para obter o nome do cliente.

7. **GROUP BY v.fk_cliente_fk_pessoa_id, p.nome**
   - `GROUP BY v.fk_cliente_fk_pessoa_id, p.nome`: Estamos agrupando os resultados com base no ID do cliente e no nome do cliente, para calcular a média de compra por cliente.
