...
('2004-05-12', 1005, 'Paulo', 'Ribeiro', 'M', 'paulo@example.com', 654, 'Bairro D', 'Ap. 303', 'Rua Velha', 38, '777.888.999-00', '6543210', NULL),
('2004-11-08', 1006, 'Juliana', 'Fernandes', 'F', 'juliana@example.com', 987, 'Bairro E', 'Casa 3', 'Rua Larga', 26, '111.222.333-00', '9876543', NULL),
...
INSERT INTO Responsavel_financeiro (conta_bancaria, fk_pessoa_id) VALUES
(987654321, 1006);
...
explicação da query:
1. **SELECT fk_produto_id AS id_produto, SUM(quantidade_produtos) AS quantidade_vendida**
   - `SELECT fk_produto_id AS id_produto`: Aqui estamos selecionando a coluna `fk_produto_id` da tabela `contem` e renomeando-a como `id_produto`.
   - `SUM(quantidade_produtos) AS quantidade_vendida`: Estamos calculando a soma da coluna `quantidade_produtos` da tabela `contem` para cada produto e renomeando-a como `quantidade_vendida`.

2. **FROM contem**
   - `FROM contem`: Estamos especificando que os dados serão selecionados da tabela `contem`.

3. **GROUP BY fk_produto_id**
   - `GROUP BY fk_produto_id`: Estamos agrupando os resultados com base no `fk_produto_id`, ou seja, estamos agrupando as vendas por produto.

4. **ORDER BY quantidade_vendida DESC**
   - `ORDER BY quantidade_vendida DESC`: Estamos ordenando os resultados com base na soma da quantidade vendida em ordem decrescente (DESC), ou seja, os produtos mais vendidos aparecerão primeiro.

5. **LIMIT 3**
   - `LIMIT 3`: Estamos limitando o resultado a apenas 3 linhas, ou seja, estamos selecionando apenas os top 3 produtos mais vendidos.
